...
// Bundled from lib/master-engine/render-worker.ts by scripts/build-worklet.ts.
...
(() => {
  ...
  function highpass(freq, q, sampleRate) {
    const w = 2 * Math.PI * clampFrequency(freq, sampleRate) / sampleRate;
    const cw = Math.cos(w);
    const alpha = Math.sin(w) / (2 * clampQ(q));
    return normalise((1 + cw) / 2, -(1 + cw), (1 + cw) / 2, 1 + alpha, -2 * cw, 1 - alpha);
  }
  function bandpass(freq, q, sampleRate) {
    const w = 2 * Math.PI * clampFrequency(freq, sampleRate) / sampleRate;
    const cw = Math.cos(w);
    const alpha = Math.sin(w) / (2 * clampQ(q));
    return normalise(alpha, 0, -alpha, 1 + alpha, -2 * cw, 1 - alpha);
  }
  function peaking(freq, gainDb, q, sampleRate) {
  ...
  function lowShelf(freq, gainDb, slope, sampleRate) {
    const A = Math.pow(10, gainDb / 40);
    const w = 2 * Math.PI * clampFrequency(freq, sampleRate) / sampleRate;
    const cw = Math.cos(w);
  ...
  function highShelf(freq, gainDb, slope, sampleRate) {
    const A = Math.pow(10, gainDb / 40);
    const w = 2 * Math.PI * clampFrequency(freq, sampleRate) / sampleRate;
    const cw = Math.cos(w);
    const s = Math.min(2, Math.max(0.1, slope));
  ...
  var EnvelopeFollower = class {
    constructor(sampleRate, attackSeconds, releaseSeconds) {
  ...
  var TransientShaper = class {
    constructor(sampleRate) {
      ...
      this.fast = new EnvelopeFollower(sampleRate, 5e-4, 0.045);
  ...
  var DeEsser = class {
    constructor(sampleRate) {
      ...
      this.follower = new EnvelopeFollower(sampleRate, 1e-3, 0.05);
      this.setCut(0);
    }
    setAmount(amount) {
      this.amount = Math.min(1, Math.max(0, amount));
  ...
  var DeChirp = class {
    constructor(sampleRate) {
      ...
      this.detectLeft.setCoefficients(c);
      this.detectRight.setCoefficients(c);
      this.fast = new EnvelopeFollower(sampleRate, 3e-4, 0.02);
  ...
  var RandomWalk = class {
    constructor(sampleRate, hz, seed) {
  ...
  var AnalogLife = class {
    constructor(sampleRate, seed = 4350) {
      __publicField(this, "sampleRate");
      ...
      this.flutter = new RandomWalk(sampleRate, FLUTTER_HZ, seed ^ 23536);
      this.gainWalk = new RandomWalk(sampleRate, 0.4, seed ^ 10977);
    }
    setAmount(amount) {
      this.amount = Math.min(1, Math.max(0, amount));
    }
  ...
  var TapeHiss = class {
    constructor(sampleRate, seed = 31326) {
      __publicField(this, "rng");
      __publicField(this, "seed");
      ...
      this.seed = seed;
      this.rng = new Rng(seed);
      const c = bandpass(6e3, 0.6, sampleRate);
  ...
  function eqSections(settings, sampleRate, channel) {
    const out = [];
    if (settings.lowCut) {
  ...
  var Limiter = class {
    constructor(sampleRate, lookaheadSeconds = DEFAULT_LOOKAHEAD_S) {
      __publicField(this, "sampleRate");
  ...
  function truePeak(channels) {
    ...
    for (const ch of channels) {
      for (let i = 0; i < ch.length; i++) {
        ...
        for (let k = 1; k < 4; k++) {
          const t = k / 4;
          const v = 0.5 * (2 * y0 + (-ym1 + y1) * t + (2 * ym1 - 5 * y0 + 4 * y1 - y2) * t * t + (-ym1 + 3 * y0 - 3 * y1 + y2) * t * t * t);
  ...
  function kWeightingShelf(sampleRate) {
  ...
  function kWeightingHighpass(sampleRate) {
  ...
  function blockMeanSquares(channels, sampleRate) {
  ...
  var LoudnessMeter = class {
    constructor(sampleRate, shortTermSeconds = 3) {
      __publicField(this, "left");
      __publicField(this, "right");
  ...
  function applyFades(channels, sampleRate, fadeInSeconds, fadeOutSeconds) {
    const length = channels.length === 0 ? 0 : channels[0].length;
    if (length === 0) return;
    const half = length / 2;
  ...
  // lib/master-engine/audit.ts
  var TRUE_PEAK_LIMIT_DB = -0.1;
  var QUIET_MARGIN_LU = 4;
  var LOUD_MARGIN_LU = 1;
  var CORRELATION_FLOOR = 0;
  var DC_LIMIT = 2e-3;
  function auditMaster(channels, sampleRate, options) {
    ...
    const severity = findings.some((f) => f.severity === "problem") ? "problem" : findings.length > 0 ? "caution" : "ok";
    return {
      severity,
      findings,
      measurements: {
  ...
  // lib/audio-io/dither.ts
  var SHAPING = [1.6, -0.8];
  var NTF = [1, -SHAPING[0], -SHAPING[1]];
  var SHAPED_POWER_GAIN = Math.sqrt(NTF.reduce((sum, h) => sum + h * h, 0));

  // lib/audio-io/wav.ts
  var MIN_SAMPLE_RATE = 4e3;
  var MAX_SAMPLE_RATE = 768e3;

  // lib/master-engine/offline.ts
  var MAX_MASTER_SECONDS = 1800;
  var RENDER_CHUNK_FRAMES = 1 << 16;
  function renderMaster(options) {
  ...
  // lib/master-engine/render-worker.ts
  function handleRenderRequest(request, post) {
    try {
      let lastPercent = -1;
