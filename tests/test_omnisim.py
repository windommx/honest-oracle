...
# ── New: OMNISIM adapter (self-consistency) + Stage-2 data loaders ───────

from omnisim import (
    OmnisimForecaster, make_omnisim_dataset,
    load_series_jsonl, load_series_csv,
)


class TestOmnisimForecaster(unittest.TestCase):
    def test_runs_through_harness(self):
        data = make_omnisim_dataset(n=6, length=14, seed=1)
        train, test = data[:3], data[3:]
        s = evaluate(OmnisimForecaster(seed=0), train, test, split_at=7)
        self.assertFalse(math.isnan(s.skill))

    def test_recovers_own_process(self):
        # Self-consistency: on data generated by OMNISIM's own dynamics, the
        # adapter (same engine) must beat persistence. If it cannot predict its
        # OWN process, the fit/predict path is broken.
        data = make_omnisim_dataset(n=8, length=16, seed=2, engine_seed=0)
        train, test = data[:4], data[4:]
        s = evaluate(OmnisimForecaster(seed=0), train, test, split_at=8)
        self.assertGreater(s.skill, 0.0)
...
class TestDataLoaders(unittest.TestCase):
    def test_jsonl_roundtrip(self):
        import os, tempfile
        path = os.path.join(tempfile.gettempdir(), "omni_series.jsonl")
        with open(path, "w") as f:
            f.write("[0.1, 0.2, 0.3]\n")
            f.write("[0.5, 0.4]\n")
        data = load_series_jsonl(path)
        self.assertEqual(data, [[0.1, 0.2, 0.3], [0.5, 0.4]])
        os.unlink(path)

    def test_csv_roundtrip(self):
        import os, tempfile
        path = os.path.join(tempfile.gettempdir(), "omni_series.csv")
        with open(path, "w") as f:
            f.write("0.1,0.2,0.3\n0.5,0.4\n")
