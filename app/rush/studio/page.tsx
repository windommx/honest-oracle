    ...
    <div className="min-h-screen bg-[#0b0e17]">
      <nav className="fixed top-0 left-0 right-0 z-50 glass-card">
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 text-xs">
            <Link href="/rush" className="text-slate-400 hover:text-[#ab5bf7]">เครื่องมือ prompt</Link>
            <Link href="/rush/dashboard" className="text-slate-400 hover:text-[#ab5bf7]">แดชบอร์ด</Link>
      ...
      <div className="pt-24 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-3xl font-bold accent-gradient">Rush Studio</h1>
          ...
          <div className="grid lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  <span className="block text-[0.7rem] text-slate-400 mb-1">ผู้ให้บริการ</span>
                  <select value={provider} onChange={(e) => setProvider(e.target.value as Provider)} className="input">
                    {PROVIDERS.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
                  </select>
                </label>
                <label className="block">
                  <span className="block text-[0.7rem] text-slate-400 mb-1">โมเดล</span>
              ...
              <label className="block">
                <span className="flex items-center justify-between text-[0.7rem] text-slate-400 mb-1">
                  <span>API key ({meta.keyHint})</span>
                  {apiKey && <button type="button" onClick={clearKey} className="text-[#ab5bf7] hover:underline">ล้าง key</button>}
                </span>
                ...
                <label className="flex items-center gap-1.5 text-[0.65rem] text-faint mt-1.5 cursor-pointer">
                  <input type="checkbox" checked={rememberKey} onChange={(e) => toggleRemember(e.target.checked)} className="accent-[#ab5bf7]" />
                  จำ key ไว้ในแท็บนี้ (ล้างเมื่อปิดแท็บ)
                </label>
              </label>
              <label className="block">
                <span className="block text-[0.7rem] text-slate-400 mb-1">System prompt (เช่น MASTER จากเครื่องมือ prompt)</span>
              ...
              <label className="block">
                <span className="block text-[0.7rem] text-slate-400 mb-1">Prompt</span>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[0.7rem] text-slate-400">ผลลัพธ์</span>
                {output && (
                  <div className="flex items-center gap-2">
                    <select value={saveLang} onChange={(e) => setSaveLang(e.target.value as "th" | "en")} className="text-[0.65rem] bg-[#0b0e17] border border-white/10 rounded px-1.5 py-0.5 text-slate-300">
                      <option value="th">ไทย</option>
                      <option value="en">EN</option>
                    </select>
                    <button onClick={saveOutput} className="text-[0.65rem] px-2 py-1 rounded border border-[#ab5bf7]/40 text-[#ab5bf7] hover:bg-[#ab5bf7]/10 flex items-center gap-1">
              ...
              <div className="bg-[#151a27] border border-white/10 rounded-xl p-4 text-sm text-slate-200 whitespace-pre-wrap min-h-[420px] max-h-[600px] overflow-y-auto">
