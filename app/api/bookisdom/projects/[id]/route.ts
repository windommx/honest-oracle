...
async function ownedProject(userId: string, id: string) {
  const project = await prisma.bookisdomProject.findUnique({ where: { id }, select: { id: true, userId: true } });
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.visibility && !body.config) {
    const current = await prisma.bookisdomProject.findUnique({ where: { id: params.id }, select: { shareToken: true } });
    const shareToken =
      body.visibility === "public" ? current?.shareToken ?? crypto.randomBytes(16).toString("hex") : current?.shareToken;
    await prisma.bookisdomProject.update({
      where: { id: params.id },
  ...
  const old = await prisma.bookisdomProjectVersion.findMany({
  ...
  if (old.length) {
    await prisma.bookisdomProjectVersion.deleteMany({ where: { id: { in: old.map((v) => v.id) } } });
  }
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.bookisdomProject.delete({ where: { id: params.id } });
  return NextResponse.json({ ok: true });
}
