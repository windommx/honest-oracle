    ...
    <div className="min-h-screen bg-[#08080e]">
      <nav className="fixed top-0 left-0 right-0 z-50 glass-card">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-6 items-center">
            ...
            <Link href="/history" className="text-[#c9a84c]">ประวัติ</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-28 pb-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-400 text-center mb-8">ดูประวัติการวิเคราะห์ทั้งหมดของคุณ</p>
          ...
          {history.length === 0 ? (
            <div className="glass-card rounded-2xl p-12 text-center">
              ...
              <p className="text-gray-400 mb-6">ยังไม่มีประวัติการวิเคราะห์</p>
              <Link
                href={getHref(activeTab)}
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#c9a84c] text-black font-medium rounded-xl hover:bg-[#d4b55d] transition-colors"
              >
            ...
            <div className="space-y-4">
              {history.map((item, index) => (
                <div key={item.id || index} className="glass-card rounded-2xl p-6 hover:border-[#c9a84c]/30 transition-colors">
                  {activeTab === "personal" && (
                    <>
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="text-right">
                          <p className="text-gray-500 text-xs">คะแนนรวม</p>
                      ...
                      <div className="mt-4 pt-4 border-t border-[#c9a84c]/10 flex gap-6 text-sm">
                        <div>
                          <span className="text-gray-500">กาลกิณี: </span>
                          <span className={item.kalaiganiScore >= 80 ? "text-green-400" : item.kalaiganiScore >= 60 ? "text-yellow-400" : "text-red-400"}>{item.kalaiganiScore}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">เลขศาสตร์: </span>
                          <span className={item.numerologyScore >= 80 ? "text-green-400" : item.numerologyScore >= 60 ? "text-yellow-400" : "text-red-400"}>{item.numerologyScore}</span>
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {activeTab === "corporate" && (
                    <>
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="text-right">
                          <p className="text-gray-500 text-xs">CAI</p>
                          <p className={`text-3xl font-bold ${
                            item.cai >= 72 ? "text-green-400" : item.cai >= 60 ? "text-yellow-400" : "text-red-400"
                          }`}>
                            {item.cai}/10
                          </p>
                        </div>
                  ...
                  {activeTab === "child" && (
                    <>
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="text-right">
                          <p className="text-gray-500 text-xs">เลขเป้าหมาย</p>
                          <p className="text-3xl font-bold text-[#c9a84c]">{item.targetNumber}</p>
                        </div>
                      </div>
                    </>
                  )}
