      ...
      <div className="max-w-3xl mx-auto px-5 py-12">
        ...
        <section className="mb-10">
          ...
          <div className="space-y-2">
            {TIERS.map((t) => (
              <div key={t.id} className="rounded-lg border border-black/10 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`ml-auto text-[0.6rem] px-1.5 py-0.5 rounded ${t.admissible ? "text-emerald-700" : "text-rose-700"}`}>
        ...
        <section className="mb-10">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-800 mb-3">
            <XCircle className="w-4 h-4 text-rose-700" /> {REFUSED_CONSTRUCTS.length} ค่าที่เราปฏิเสธจะให้คะแนน — และเหตุผล
          </h2>
          <div className="grid gap-1.5">
            {REFUSED_CONSTRUCTS.map((c) => (
              <div key={c.id} className="text-[0.72rem] text-slate-600 rounded border border-rose-500/15 bg-rose-500/[0.03] px-3 py-1.5">
        ...
        <section>
          ...
          <div className="rounded-lg border border-black/10 p-3 mb-3 text-[0.72rem] text-slate-600 leading-relaxed">
            <span className="text-slate-800 font-medium">{registered}</span> แหล่งอ้างอิงลงทะเบียนแล้ว จาก
            ~<span className="text-slate-800 font-medium">{mentionsEstimate ?? "…"}</span> การอ้างอิงในโมดูล (ประมาณการแบบนับเกิน).{" "}
            <span className={primaryCount === 0 ? "text-rose-700" : "text-emerald-700"}>
          ...
          <h3 className="text-xs font-semibold text-rose-700 mb-2">
            ...
            {disputed().map((c) => (
              <div key={c.id} className="rounded-lg border border-rose-500/15 bg-rose-500/[0.03] p-3">
                <p className="text-[0.75rem] text-slate-800">
                  {c.who} <span className="text-faint">({c.year})</span> — <span className="text-slate-600">{c.claim}</span>
                </p>
        ...
        <p className="mt-12 text-[0.65rem] leading-relaxed text-faint border-t border-black/5 pt-5">
