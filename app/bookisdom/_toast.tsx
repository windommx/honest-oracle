      ...
      {items.map((t) => {
        ...
        return (
          <div
            ...
            className={`flex items-start gap-2.5 rounded-xl border ${tone.border} bg-[#ffffff] px-3.5 py-3 shadow-lg shadow-black/40 backdrop-blur`}
          >
            <span className={`mt-1.5 inline-block h-2 w-2 shrink-0 rounded-full ${tone.dot}`} aria-hidden />
            <p className="flex-1 text-[0.8rem] leading-snug text-slate-800">{t.message}</p>
            <button
              onClick={() => dismissToast(t.id)}
              aria-label="ปิดการแจ้งเตือน"
              className="shrink-0 text-faint hover:text-slate-800 transition text-sm leading-none px-1"
            >
              ✕
            </button>
          </div>
