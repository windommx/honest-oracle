...
describe("keyboard accessibility (WCAG 2.4.7 Focus Visible)", () => {
  it("a global :focus-visible ring exists — it did not before this pass", () => {
    ...
    // app/bookisdom, while `focus:outline-none` appeared 7 times, actively removing the
  ...
  it("every className=\"input\" resolves to a real style rule", () => {
    // `.input` was used 11 times across /bookisdom and /bookisdom/studio and was defined NOWHERE —
    // not in globals.css, not in tailwind.config. Those fields rendered with no border,
    // background or padding at all.
    const users = tsxFiles(BOOKISDOM_DIR).filter((f) => readFileSync(f, "utf8").includes('className="input"'));
  ...
  it("no element removes the focus outline without the global ring covering it", () => {
    ...
    const strippers = tsxFiles(BOOKISDOM_DIR).filter((f) => readFileSync(f, "utf8").includes("focus:outline-none"));
...
describe("screen-reader labelling", () => {
  it("every icon-only button carries an accessible name", () => {
    ...
    for (const f of tsxFiles(BOOKISDOM_DIR)) {
...
describe("mobile layout", () => {
  it("the /bookisdom nav wraps instead of overflowing a narrow screen", () => {
    ...
    const nav = readFileSync(join(BOOKISDOM_DIR, "page.tsx"), "utf8");
...
describe("bundle discipline", () => {
  it("the light UI primitives import no engine analyzer", () => {
    ...
    // engine import lands here, /bookisdom's initial bundle silently grows again and the
    ...
    const ui = readFileSync(join(BOOKISDOM_DIR, "_ui.tsx"), "utf8");
    const imports = Array.from(ui.matchAll(/^import .*from "([^"]+)";$/gm)).map((m) => m[1]);
    const engineImports = imports.filter((i) => i.includes("bookisdom-engine") && !i.endsWith("/types"));
    expect(engineImports, `_ui.tsx must stay analyzer-free, found: ${engineImports.join(", ")}`).toEqual([]);
  });

  it("/bookisdom loads the heavy modals dynamically, not statically", () => {
