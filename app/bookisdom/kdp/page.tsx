    ...
    <div className="min-h-screen">
      ...
      <div className="pt-24 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-slate-600 mt-1 text-sm mb-6 max-w-2xl">
            สันปก ขนาดปกเต็ม และเช็กลิสต์ metadata จากสูตรที่ Amazon เผยแพร่ — ไม่มีคะแนน มีแต่กฎที่ผ่านหรือไม่ผ่าน
            จำนวนหน้าเป็น<span className="text-slate-700">ค่าประมาณ</span> (คำ ÷ คำต่อหน้าของขนาดเล่ม) ตรวจซ้ำใน KDP previewer เสมอ
          </p>

          <div className="grid lg:grid-cols-2 gap-5">
            {/* left: inputs */}
            <div className="space-y-4">
              <section className="rounded-xl border border-black/10 bg-[#ffffff] p-4 space-y-3">
                ...
                <label className="block">
                  <span className="block text-[0.7rem] text-slate-600 mb-1">จำนวนคำ {counting && <span className="text-faint">(กำลังนับ…)</span>}</span>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <label className="block">
                    <span className="block text-[0.7rem] text-slate-600 mb-1">ขนาดเล่ม (นิ้ว)</span>
                    <select value={trim} onChange={(e) => setTrim(e.target.value as TrimSize)} className="input" aria-label="ขนาดเล่ม">
                      {TRIMS.map((t) => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </label>
                  <label className="block">
                    <span className="block text-[0.7rem] text-slate-600 mb-1">กระดาษ</span>
                  ...
                  <label className="block">
                    <span className="block text-[0.7rem] text-slate-600 mb-1">การเข้าเล่ม</span>
              ...
              <section className="rounded-xl border border-black/10 bg-[#ffffff] p-4 space-y-3">
                ...
                <label className="block">
                  <span className="flex justify-between text-[0.7rem] text-slate-600 mb-1"><span>ชื่อเรื่อง</span><span className="tabular-nums">{(meta.title ?? "").length}/{KDP_LIMITS.title}</span></span>
                  <input value={meta.title} onChange={(e) => setMeta((s) => ({ ...s, title: e.target.value }))} className="input" aria-label="ชื่อเรื่อง" />
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="flex justify-between text-[0.7rem] text-slate-600 mb-1"><span>ชื่อรอง</span><span className="tabular-nums">{(meta.subtitle ?? "").length}/{KDP_LIMITS.subtitle}</span></span>
                    <input value={meta.subtitle} onChange={(e) => setMeta((s) => ({ ...s, subtitle: e.target.value }))} className="input" aria-label="ชื่อรอง" />
                  </label>
                  <label className="block">
                    <span className="block text-[0.7rem] text-slate-600 mb-1">ผู้เขียน</span>
                    <input value={meta.author} onChange={(e) => setMeta((s) => ({ ...s, author: e.target.value }))} className="input" aria-label="ผู้เขียน" />
                  </label>
                </div>
                <label className="block">
                  <span className="flex justify-between text-[0.7rem] text-slate-600 mb-1"><span>คำโปรย</span><span className="tabular-nums">{desc.trim().length}/{KDP_LIMITS.description} (ขั้นต่ำ {KDP_LIMITS.descriptionMin})</span></span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="flex justify-between text-[0.7rem] text-slate-600 mb-1"><span>คีย์เวิร์ด (คั่นด้วย , หรือขึ้นบรรทัดใหม่)</span><span className="tabular-nums">{kw.length}/{KDP_LIMITS.keywords}</span></span>
                  ...
                  <label className="block">
                    <span className="flex justify-between text-[0.7rem] text-slate-600 mb-1"><span>หมวดหมู่</span><span className="tabular-nums">{cats.length}/{KDP_LIMITS.categories}</span></span>
            ...
            {/* right: results */}
            <div className="space-y-4">
              <section className="rounded-xl border border-black/10 bg-[#ffffff] p-4">
                ...
                {hasWords && r.spine.inches < 0.06 && (
                  <p className="mt-3 text-xs text-amber-800 flex items-start gap-1.5"><AlertTriangle className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />สันบางกว่า 0.06 นิ้ว — KDP ไม่ให้ใส่ตัวอักษรบนสัน</p>
                )}
              </section>
              ...
              <section className="rounded-xl border border-black/10 bg-[#ffffff] p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs tabular-nums ${failing === 0 ? "text-[#166534]" : "text-amber-800"}`} data-testid="checklist-summary">
                ...
                <ul className="space-y-2 text-sm" aria-label="เช็กลิสต์ KDP">
                  {checks.map((c) => (
                    <li key={c.rule} className="flex items-start gap-2">
                      <span className={`mt-0.5 w-4 h-4 rounded flex-shrink-0 flex items-center justify-center text-[0.6rem] ${c.ok ? "bg-[#166534]/15 text-[#166534]" : "bg-[#b91c1c]/15 text-[#b91c1c]"}`} aria-hidden="true">{c.ok ? "✓" : "✕"}</span>
                      <span className="min-w-0">
                        <span className="text-slate-800">{c.rule}</span>
                ...
                <p className="mt-3 text-[0.7rem] text-faint">
                  {!hasWords && <span className="text-amber-800">กฎด้านการพิมพ์ ({r.checks.length - checks.length} ข้อ) จะปรากฏเมื่อใส่จำนวนคำ · </span>}
              ...
              <section className="rounded-xl border border-black/10 bg-[#ffffff] p-4">
                ...
                <div className="grid sm:grid-cols-2 gap-3 text-[0.75rem]">
                  <div>
                    <div className="text-slate-700 mb-1">ต้องเปิดเผย (อยู่ในเล่ม)</div>
                    <ul className="list-disc pl-4 text-slate-600 space-y-0.5">{KDP_AI_DISCLOSURE.mustDisclose.map((x) => <li key={x}>{x}</li>)}</ul>
                  </div>
                  <div>
                    <div className="text-slate-700 mb-1">ไม่ต้องเปิดเผย (งานช่วยเบื้องหลัง)</div>
                    <ul className="list-disc pl-4 text-slate-600 space-y-0.5">{KDP_AI_DISCLOSURE.noDisclosure.map((x) => <li key={x}>{x}</li>)}</ul>
                  </div>
                </div>
              ...
              <p className="text-[0.7rem] text-faint leading-relaxed">
                <span className="text-slate-700">สิ่งที่หน้านี้ทำไม่ได้:</span> ไฟล์เนื้อในพร้อมพิมพ์ (PDF ฝังฟอนต์ / CMYK) ต้องทำในโปรแกรมจัดหน้า — เบราว์เซอร์ทำ CMYK ที่แม่นยำไม่ได้ Atticus และ BookyAI ทำส่วนนั้น เราไม่ทำ
              </p>
            </div>
          </div>
        </div>
