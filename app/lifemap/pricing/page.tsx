...
type UsageResp = {
  plan: string;
  role: string;
  today: { lifemapReads: number; apiCalls: number };
  limits: { lifemapDaily: number | null; apiDaily: number | null };
};
...
export default function LifemapPricingPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const res = await fetch("/api/lifemap/usage");
  ...
  const plans: PlanItem[] = [
    {
      ...
      cta: { label: "เริ่มใช้งาน", href: "/lifemap/app" },
    },
    {
      key: "pro",
      name: "Pro",
    ...
    },
    {
      ...
      cta: { label: "ขอ Premium", href: "/lifemap/admin" },
    },
  ];
      ...
      <div className="pt-28 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-600">เริ่มต้นฟรี แล้วอัปเกรดเมื่อพร้อม</p>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 mb-8">
            ...
            {msg && <p className="text-red-700 text-sm text-center mt-3">{msg}</p>}
          </div>
