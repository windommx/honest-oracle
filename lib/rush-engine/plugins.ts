...
// ║  A Rush plugin is DATA, never code: a manifest + prompt modules +  ║
...
/** Validate untrusted JSON into a RushPlugin. Pure; reads ONLY known fields, so any
 *  extra keys (including anything that looks like code) are ignored, not executed. */
export function validatePlugin(raw: unknown): { ok: true; plugin: RushPlugin } | { ok: false; errors: string[] } {
  const errors: string[] = [];
...
export function pluginPrompts(plugins: RushPlugin[]): PluginPrompt[] {
...
export function pluginLexiconTerms(plugins: RushPlugin[], lang: "th" | "en"): { sensory: Record<Sense, string[]>; aiTells: string[] } {
