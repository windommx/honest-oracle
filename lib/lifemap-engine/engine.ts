import { LifemapInput, LifemapResult, LifemapTone } from "@/lib/lifemap-engine/types";
...
function scoreTone(score: number): LifemapTone {
...
function buildEventAnchors(timeline: LifemapResult["timeline"]) {
  ...
  const turning: Array<{ age: number; tone: LifemapTone; score: number }> = [];
...
export function generateLifemapReading(input: LifemapInput): LifemapResult {
  const seedSource = [
