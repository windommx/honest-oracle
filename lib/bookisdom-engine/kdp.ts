...
export const KDP_AI_DISCLOSURE = {
  ...
  /** How Bookisdom's OWN output maps onto the line — stated plainly, not hidden. */
  bookisdomNote:
    "Bookisdom สร้าง prompt + วิเคราะห์แบบนับได้ ตัวมันเองไม่ได้เขียนเนื้อในเล่ม: ถ้าคุณเอา prompt ไปให้ LLM " +
    "เขียนข้อความแล้วใช้เป็นเนื้อในเล่ม = AI-generated ต้อง disclose; ถ้าคุณเขียนเองแล้วใช้ Bookisdom ช่วยวิเคราะห์/แก้ = AI-assisted ไม่ต้อง disclose. " +
    "ไม่ว่าทางไหน ให้ประกาศตามจริง — Bookisdom ไม่สอนวิธีหลบเครื่องตรวจ (ผิดกฎ KDP และขัดกับหลักการทั้งหมดของเครื่องมือนี้).",
} as const;
...
export function formatKdpPackage(input: { words: number; trim?: TrimSize; paper?: PaperWeight; binding?: "paperback" | "hardcover"; meta?: KdpMeta }): string {
  ...
  L.push("", `> ${KDP_AI_DISCLOSURE.bookisdomNote}`);
