...
const HELP = `bookisdom — deterministic novel-writing engine (no LLM, no network)

USAGE
  bookisdom prompts  --type <t> --genre <g> [--chapters n] [--words n] [--lang th|en] [--full]
  bookisdom analyze  <file.md> [--lang th|en]
  bookisdom rename   <file.md> --from <name> --to <name> [--lang th|en] [--write]
  bookisdom relations <file.md> --names "A,B,C" [--lang th|en]
  bookisdom radar    <file.md> --canon "A,B,C" [--lang th|en]
  bookisdom codex    <draft.md> --bible <bible.md> [--lang th|en]  (or: --bible <b> --graph)
  bookisdom saga     --books "b1.md,b2.md,..." [--titles "A,B,..."] [--lang th|en]
  bookisdom openers  <file.md> [--lang th|en]   (sentence/clause opener monotony)
  bookisdom excess   <suspect.md> --baseline <human.md> [--lang th|en] [--min n]
  bookisdom scene    <file.md>   (Thai: real per-scene signals — no fake 0–100 scores)
  bookisdom narrative <file.md> [--names "A,B"] [--motifs "x,y"]  (Thai: presence/pacing/motifs)
  bookisdom route    "<อาการที่เจอ>"   (symptom → which of the 61 modules to open)
  bookisdom receipt  <file.md> [--lang th|en] [--verify]  (measurement receipt + re-derive check)
  bookisdom cite     [--module <ID>] [--recheck]   (citation ledger: how strongly each source was checked)
  bookisdom help
...
function cmdAnalyze(file: string | undefined, flags: Record<string, string | true>, read?: (p: string) => string): CliResult {
  if (!file) return { stdout: "", stderr: "analyze needs a file: bookisdom analyze <file.md>\n", code: 2 };
  ...
  const L: string[] = [`# bookisdom analyze — ${file} (${lang})`, ""];
...
function cmdRoute(symptom: string): CliResult {
  if (!symptom.trim()) {
    return { stdout: "", stderr: `bookisdom route "<อาการที่เจอ>"\nเช่น: bookisdom route "จบบทแล้ววางได้ ไม่มีใครอ่านต่อ"\n`, code: 2 };
  }
...
function cmdReceipt(file: string | undefined, flags: Record<string, string | true>, read?: (p: string) => string): CliResult {
  ...
  const reproduce = `npm run bookisdom -- receipt ${file} --lang ${lang}`;
...
export function runCli(argv: string[], io?: { read?: (path: string) => string }): CliResult {
  ...
  return { stdout: "", stderr: `unknown command "${cmd}". try: bookisdom help\n`, code: 2 };
}
