...
// ║                                                                    ║
// ║  Bookisdom ships 61 modules. Until now the only way in was              ║
...
// ║  to satisfy Bookisdom's own contract — table-driven, first-match-wins,  ║
...
export const SYMPTOM_LADDER: Rung[] = [
  ...
  },
  {
    ...
    why: "Distance is adjustable and measurable-adjacent: the psychic-distance ladder names which rung the prose sits on, and sensory density is a count Bookisdom actually reports.",
  },
  {
    id: "R7",
    ...
    why: "Bookisdom counts filter verbs and named emotions per 100 words, so this symptom has a direct Tier-1 readout — fix it against the count, not against a vibe.",
  },
  {
    id: "R8",
  ...
  },
  {
    ...
    why: "Bookisdom already tracks threads that are introduced and then never traced again — this rung points at that existing audit.",
  },
  {
    id: "R23",
    ...
    why:
      "Bookisdom counts theme-term occurrences per chapter, so 'the motif disappears after chapter 9' " +
  ...
  },
  {
    ...
    why: "Bookisdom counts dialogue ratio and repeated-line fatigue; the module works against those counts.",
  },
  {
    id: "R13",
    ...
    why: "Bookisdom matches a fixed cliché lexicon, so the hits are re-derivable line by line. The score for 'is this AI-written' is refused — the LIST of hits is not.",
  },
  {
    id: "R14",
  ...
  },
  {
    ...
    why: "Listing craft is separable from manuscript craft. Bookisdom reports what the metadata contains — never a projected sales rank.",
  },
];
...
export const NON_GOALS: { what: string; why: string }[] = [
  {
    ...
    why:
      ...
      "copy. It also conflicts head-on with Bookisdom's contract: a router that learns returns different " +
      "routes for the same sentence over time, which is exactly the non-reproducibility we refuse " +
      "in LLM scoring. If Bookisdom ever learns from usage, it will be a versioned ladder edit in git " +
      "that you can diff — not a silent mutation.",
  },
  {
    what: "AI Bootstrap — installing toolchains and MCP servers on demand",
    why:
      "Coherent for a pentest pack that needs IDA or Frida present. Bookisdom emits text and makes zero " +
  ...
  },
  {
    ...
    why:
      ...
      "manuscript has no scope boundary to breach. Bookisdom's equivalent already exists and is narrower: " +
