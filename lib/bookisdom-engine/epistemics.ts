// ╔══════════════════════════════════════════════════════════════════╗
// ║  EPISTEMICS — the theory of knowledge Bookisdom is built on.           ║
// ║  ญาณวิทยาของ Bookisdom: every signal the engine emits is classified by  ║
...
// ║  measures the judge, not the writing. Bookisdom refuses to print it.    ║
...
/** The four tiers of knowing, strongest → weakest → refused. Each Bookisdom signal
...
export interface TierInfo {
  ...
  admissible: boolean; // may Bookisdom present it as knowledge at all?
...
export const TIERS: TierInfo[] = [
  ...
  },
  {
    ...
    warrant:
      ...
      "and temperature: a measurement of the judge, not the text. Bookisdom refuses to print it.",
  },
];
...
/** The registry: every signal Bookisdom surfaces, tagged with its epistemic status. Edit a
...
/** Constructs Bookisdom deliberately REFUSES to score — with the named fallacy each would
 *  commit. This is the boundary the competition crosses and Bookisdom does not. */
...
export const REFUSED_CONSTRUCTS: RefusedConstruct[] = [
  ...
  { id: "aiSlopScore", thai: "คะแนน AI-slop", why: "same error as isAiWritten with a decimal added ('0.02'): an inference sold as detection — Bookisdom counts cliché-lexicon hits instead, which you can re-derive line by line" },
];
...
/** Is this signal something Bookisdom may present AS knowledge? True for Tier 1–3 (all
