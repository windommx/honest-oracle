...
- **ล็อกช่วงเก็บผลจริง:** แท็บ Evidence → ล็อกกติกา (หรือ `POST /api/research/freeze`) เก็บ sha256 ของ config_th, signals/stops policy, meta_model และ prereg · ระหว่างล็อก GET ไม่บันทึก policy, `PUT /api/config/th` / seed / replaceDemo / reset prereg / deploy meta_model ได้ 409, Evidence Night รันแบบอ่านอย่างเดียว และถ้าค่าถูกแก้ข้ามด่านจะขึ้นเตือนทั้งแท็บ Evidence และ Track Record (ปลดล็อกต้องพิมพ์ `UNFREEZE` + เหตุผล)
