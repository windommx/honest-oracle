...
**กติกากับข้อมูลจริง (มีรู / ขาด OHLC / หยุดพัก / IPO):**

- แท่งที่ไม่มี open/high/low **ไม่ถูกใช้เป็นราคา** — ไม่เป็น swing/sweep/ขอบ FVG · High/Low 20 วัน/52 สัปดาห์นับเฉพาะแท่งที่มี high/low จริง · Volume Profile ใช้ราคาปิดของแท่งนั้นแทน · FVG ถือว่าถูก mitigate จากแท่งแบบนี้เมื่อ "ราคาปิด" แตะโซนแล้วเท่านั้น · แท่ง**ล่าสุด**ไม่มี OHLC = ชั้น Location/Value ปิด (ตามข้อ §2)
- แท่งแบน (high = low) ไม่ได้ "ปิดแข็งฝั่งใด" → ไม่นับ absorption
- ตลาดของ Daily Brief/Circuit Breaker = equal-weight บน**ปฏิทินตลาด**: วันไหนนับเฉพาะหุ้นสภาพคล่องผ่านที่มีแถวทั้งวันนั้นและวันก่อนหน้า (breadth: วันนั้นและ 20 วันก่อน) — วัดไม่ได้แสดง "—" ไม่เติม 0% / 50%
- watchlist ใช้มูลค่าเฉลี่ย 20 **วันตลาด**ล่าสุด (วันที่หุ้นถูกพัก = 0) → หุ้นที่หยุดซื้อขายไม่ค้างใน checklist ด้วยสภาพคล่องเก่า · Sector Rotation นับเฉพาะหุ้นที่มีแถววันล่าสุด · ถัง Unknown/OTHER แสดงในตารางได้แต่ไม่เป็น "ผู้นำกลุ่ม"
- Lead-lag: asset ที่วันที่ทับกับตลาดไทยไม่พอ (< 60 คู่) ไม่ออกแถว (ไม่แสดง r ปลอม)
