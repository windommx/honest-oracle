...
- **Self-test 14 invariant (รันสดทุกครั้งที่เปิดแท็บ)** — ไม่มี look-ahead (1.01^n เป๊ะ + กับดักเดือน crash −20% ที่ยังไม่รู้ล่วงหน้าต้องโดนเต็ม) · SMA/12-1 ตรงคำตอบปิดรูป · หุ้นขาลง monotone ถูกเตะไปเงินสดทุกเดือน · crash→cash ทำ MaxDD ตื้นกว่าถือตายตัวชัดเจน · ต้นทุน monotonic · น้ำหนักรวม 100% ทุกเดือน · หน้าต่าง walk-forward ไม่ทับกัน · Monte Carlo deterministic ต่อ seed · cashMode trendedBond เดินถูกทางทั้งสองกรณี · universe 13 ตัวตรงรายชื่อ · **คณิตเดือน (nextMonthLabel/monthDiff ข้ามปี) · Macro Gate stance ทั้ง 3 ระดับ · Tracking evaluation ตรงราคาปิดรูป (รวมส่วนเงินสด) + สัญญาณที่เดือนใช้ยังไม่ปิดไม่ให้คะแนน**
...
|---|---|---|---|---|
| **GTAA Top 6 · SMA 10** | **+7.4%** | **−14.9%** | **0.89** | +8.3% |
| SPY ถือตายตัว | +9.4% | −50.8% | 0.62 | — |
| GTAA Top 9 · SMA 10 (12-1) | +5.6% | **−13.8%** | **0.85** | +6.5% |

> **แก้ตัวเลข (2026-09):** ชุดก่อนหน้า (Top 6: +6.5% / −16.9% / 0.79 · turnover 1,209%/ปี) มาจากบั๊ก backtester — น้ำหนักของตัวที่ขายไปแล้วค้างอยู่ใน tranche จึงถูกคิดต้นทุน "ขาย" ซ้ำทุกเดือน (และเมื่อ tranches > 1 ยังได้ผลตอบแทนของตัวที่ขายไปแล้วต่อ) · หลังแก้ turnover จริง = 395%/ปี (Top 6) และ 304%/ปี (Top 9 · 12-1) ตรงกับ implementation อ้างอิงอิสระทุกเดือน

- Sensitivity บนข้อมูลจริงยังยืนยันข้อค้นพบเดิม: **Top N คือปุ่มเสี่ยงหลัก** (Top 3 Sharpe 0.72–0.73 / DD −18…−22% · Top 4–8 Sharpe 0.88–0.91 · Top 9 Sharpe 0.86–0.87 / DD −10…−13% — N มากขึ้น DD ตื้นลงแต่ CAGR ลดจาก +8.9% (Top 4) เหลือ +5.7% (Top 9)) ส่วน SMA 8/10/12 ต่างกันน้อยมาก (Sharpe ในแถวเดียวกันห่าง ≤ 0.02) — **อย่าเสียเวลาจูนความยาว SMA**
- Walk-forward 23 หน้าต่าง: OOS degradation (median) 0%, config ที่ถูกเลือกบ่อยสุด = Top 6 · SMA 10 (30% ของหน้าต่าง) — แต่หน้าต่างช่วง GFC (2007-12→2008-11) Sharpe หาย ~80% ตามสภาพตลาดจริง
...
3. **อัปโหลด CSV** — รองรับ wide (`Date,VTV,MTUM,…`) และ long (`date,ticker,adjclose`) — ไม่ผ่าน gate = ไม่บันทึก (เหตุผลแสดงครบ) · long ที่มีทั้ง Close และ Adj Close ใช้ Adj Close เสมอ · ไฟล์รายวัน (เรียงลำดับไหนก็ได้) ใช้ราคาวันท้ายสุดของเดือน · รองรับเซลล์ในเครื่องหมายคำพูด/BOM/CRLF · จำกัด 4 ล้านตัวอักษร · ไฟล์ไม่มี BIL/SPY จะเติมชุดแทนพร้อมหมายเหตุบนการ์ดข้อมูล

**Quality gate (Step 0.2 ของแผน):** ห้ามรูกลางซีรีส์ (รวมกริดเดือนที่ขาดทั้งแถว) · ราคา > 0 · กระโดด |ln(c/c_prev)| > 0.5 = แจ้ง split ที่ไม่ถูก adjust · เดือนล่าสุดต้องตรงกัน · ≥ 24 เดือนต่อตัว — `hole/nonpositive/jump` บล็อกการบันทึก, `misaligned/short` เตือน
...
- **การประเมินย้อนหลังอัตโนมัติ** (`tracking.ts`): เมื่อ panel มีข้อมูลเดือนถัดไปแล้ว snapshot แต่ละแถวได้ผลจริง `portfolio = Σ w·r` (holdings + ส่วนเงินสดที่ cashTicker — สัญญาณเงินสด 100% ก็ได้คะแนน) เทียบ SPY → delta, hit/miss · สัญญาณที่เดือนใช้ยังไม่ปิด = "รอผล" (ไม่ให้คะแนน) — "ปิด" = เดือนตามปฏิทิน (UTC) จบแล้ว **และ** panel ถูกดึง/อัปโหลดหลังเดือนนั้นปิด (แท่งเดือนปัจจุบันจาก Yahoo ยังเป็นราคากลางเดือน) · snapshot จากข้อมูลจริงไม่ตรวจกับ panel สังเคราะห์ (ขึ้น "ขาดข้อมูล")
