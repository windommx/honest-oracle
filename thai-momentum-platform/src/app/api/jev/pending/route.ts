...
export async function POST(req: Request) {
  try {
    ...
    if (approve) {
      ...
      if (row.question === "Q_ENTRY" && row.action === "buy" && isFinite(lastPx)) {
        const existing = await db.position.findUnique({ where: { symbol: row.target } })
        if (!existing) {
          const slots = row.conf >= 0.85 ? 1.0 : 0.5
          ...
          message = `อนุมัติและเข้าพอร์ต ${row.target} ที่ราคา ${lastPx.toFixed(2)} (slots ${slots})`
        } else {
