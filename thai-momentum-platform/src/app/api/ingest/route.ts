...
import { backupBeforeOverwrite, type BackupInfo } from "@/lib/security/destructive-guard"
...
// upsert เท่านั้น (ไม่ลบแถว) — แต่ถ้าช่วงวันที่ในไฟล์ทับข้อมูลที่มีอยู่ (แก้ย้อนหลัง/นำเข้าซ้ำ) จะสำรอง DB ก่อนเขียนทับ
export async function POST(req: Request) {
  try {
    ...
    const backup: BackupInfo | null = await backupBeforeOverwrite(
      parsed.rows.map((r) => r.date),
      "ingest-overwrite",
    )
    ...
    return NextResponse.json<IngestResponse & { backup?: BackupInfo }>({
      ok: true,
      insertedRaw: sum.insertedRaw,
      updatedRows: sum.updatedRows,
      snapDates: sum.snapDates,
      tookMs: Date.now() - t0,
      message: `รับข้อมูล ${parsed.kind === "history" ? "แบบประวัติย้อนหลัง" : "แบบ snapshot รายวัน"} ${sum.insertedRaw} แถว / ปรับปรุง indicator ${sum.updatedRows} แถว / สร้างโผใหม่ ${sum.snapDates.length} วัน${backup ? ` · สำรอง DB ก่อนเขียนทับไว้ที่ ${backup.file}` : ""}`,
      ...(backup ? { backup } : {}),
    })
  } catch (e) {
